...
import { SectionTitle, SectionPageTitle } from "src/components/Section";
import DefaultButton from "../Button/DefaultButton";

const RequestForm = ({ location, thanksURL, sourceURL, cartItems = [] }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      const data = {
        ...
        CART ITEMS:
        ${cartItems.map(item =>
          ` ${item.title} (SKU: ${item.sku})  Qty: ${item.qty || 1}  Price: $${item.price.toLocaleString()}  Subtotal: $${(item.price * (item.qty || 1)).toLocaleString()}`
        ).join('\n')}
        
        CART TOTAL: $${cartItems.reduce((sum, item) => sum + (item.price * (item.qty || 0)), 0).toLocaleString()}
        `,
      };
      try {
  ...
  return (
    <form id="checkout-form" onSubmit={handleSubmit}>
      <Form className="form">
        <FormHeader>
          <SectionPageTitle textAlign="left" className="h3">Billing Details</SectionPageTitle>
          <Link to="/cart"><DefaultButton text="Back" size="sm"></DefaultButton></Link>
        </FormHeader>
        <FormRow className="form-row">
          ...
          <FormCol>
            <SectionTitle $textColor="#fff" className="h4 tw-pt-[15px]">Additional Information</SectionTitle>
          </FormCol>
          <FormCol>
            <div className="form-group">
              <textarea

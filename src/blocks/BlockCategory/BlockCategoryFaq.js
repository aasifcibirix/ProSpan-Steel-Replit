  ...
  return (
    <Section className="section-faq" $bgcolor="#fff">
      <Container $maxWidth="1680px">
        <Row>
          <ColLeft>
            <SectionTitle>{data.productCategory.faqHeading}</SectionTitle>
            {data.productCategory.faqSubHeading &&
              <SectionDescription dangerouslySetInnerHTML={{ __html: data.productCategory.faqSubHeading.childMarkdownRemark.html }} />
            }
            <Figure>
          ...
          <ColRight>
            <Accordions className="accordions">
              {data.productCategory.faqCollections && data.productCategory.faqCollections.map(d => (
                <Accordionsitems

  ...
  return (
    <Section className="section-faq" $bgcolor="#fff">
      <Container $maxWidth="1680px">
        <Row>
          <ColLeft>
            <SectionTitle>{data.state.faqTitle}</SectionTitle>
            <SectionDescription dangerouslySetInnerHTML={{ __html: data.state.faqDescription.childMarkdownRemark.html }} />
            <Figure>
              <GatsbyImage image={data.image.gatsbyImageData} alt={data.state.faqTitle.textShort} />
            </Figure>
          </ColLeft>
          <ColRight>
            <Accordions className="accordions">
              {data.state.faqCollections && data.state.faqCollections.map(d => (
                <Accordionsitems
